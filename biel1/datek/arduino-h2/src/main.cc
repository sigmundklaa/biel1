...
void onDataTransmitted(
    const uint8_t* receiverMacAddress, esp_now_send_status_t transmissionStatus
);
void onDataReceived(
    const uint8_t* senderMacAddress, const uint8_t* receivedData,
...
esp_now_peer_info_t fanDeviceInfo = {
    .peer_addr =
        {0x0C, 0x8B, 0x95, 0x42, 0xFD,
         0xA0}, // Change this address to the address of the Fan
    .channel = 0,
    .encrypt = false,
};
...
void
connectToEspNowPeer(esp_now_peer_info_t& peerInfo)
{
    // Avoid people using the example without changing the peer address
    if (peerInfo.peer_addr[0] == 0xFF) {
        Serial.println("Please change the peer mac address to the address of "
                       "the other ESP32 at the top of the code");
...
void
sendData(DynamicJsonDocument jsonDoc, esp_now_peer_info_t& peerInfo)
{
    ...
    esp_err_t status =
        esp_now_send(peerInfo.peer_addr, payloadBytes, payloadSize);
...
// The * and & symbols can mostly be ignored for our cases. Its arrays,
// references and pointers in the C-language used in specific cases.
void
onDataReceived(
    const uint8_t* senderMacAddress, const uint8_t* receivedData,
    int receivedDataLength
)
{
    ...
    DeserializationError error =
        deserializeJson(receivedJson, receivedData, receivedDataLength);

    // If there was an error converting the received data to json, print the
    // error message and exit the function
...
void
resendReceivedJson(DynamicJsonDocument& jsonDoc)
{
    ...
    Serial.println("reciever: " + receiver);
...
void
printEspErrorCode(String message, esp_err_t errorCode)
{
    Serial.println(message);
    Serial.print("Error Code:");
    Serial.println(errorCode, HEX);
    Serial.println("The meaning of this error code can be found at:");
    Serial.println("https://docs.espressif.com/projects/esp-idf/en/latest/"
                   "esp32/api-reference/error-codes.html");
}
